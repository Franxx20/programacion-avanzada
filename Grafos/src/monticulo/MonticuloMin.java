...
public class MonticuloMin<T extends Comparable<? super T>> implements ColaDePrioridad<T> {
    private T[] heap;
    private int size;
    private Comparator<T> comparator;

    public MonticuloMin(int capacidad) {
        heap = (T[]) new Comparable[capacidad + 1];
        size = 0;
    }

    public MonticuloMin(T[] items) {
     ...
     * @param i
     * @param j
     * @return true si el i es mayor al j, false no es mayor
     */
    ...
    private void sortUp(int ind) {
        while (ind > 1 && isBigger(ind / 2, ind)) {
            swap(ind / 2, ind);
            ind = ind / 2;
        }
    }
    ...
    private static void sink(Comparable[] a, int k, int N) {
        while (2 * k <= N) {
            int j = 2 * k;
            if (j < N && bigger(a, j, j + 1)) j++;
            if (!bigger(a, k, j)) break;
            exch(a, k, j);
            k = j;
        }
    ...
    private static boolean bigger(Comparable[] a, int i, int j) {
